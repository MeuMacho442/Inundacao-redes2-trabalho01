...
import Controller.Excessao.ExcessaoIncosistenciaArquivo;
import View.Janela;
import Model.Node;
...
public class AlocarNode {
     ...
     public void setNode() {
         ...
         try {  
             ...
             for(int i = 0; i < tamanho; i++) {
                   ...
                   addNode((node.getSegundoValor()).getX(), (node.getSegundoValor()).getY(), (i+1));
             }
         } catch(Exception ex) {
               ex.printStackTrace();
         }
     ...
     ********************************************************************************************/
     public void setAresta() throws ExcessaoIncosistenciaArquivo {
         ...
         for(int i = 1; i < tamanho; i++) {//comecamos por um porquer as informacoes relevantes para estruturar o grafo esta no arquivo backbone.txt
            ...
            int peso = arquivoSubRede.extrairNumeroDocumentoSubrede(i, 2);
     ...
     * Retorno: vazio
     ********************************************************************************************/ 
     private void addNode(double x, double y, Integer valor) {
         subRede.alocarNodeView(valor, x, y);
     } 
     ...
     * Retorno: vazio
     ********************************************************************************************/
     private void addAresta(Node<Roteador, Posicao> w, Node<Roteador, Posicao> r, int peso) {
         try {   
            grafo.inserirAresta(w,r,peso);
            subRede.alocarArestaView(peso, (w.getSegundoValor()).getX(), (w.getSegundoValor()).getY(), (r.getSegundoValor()).getX(), (r.getSegundoValor()).getY());
         } catch(Exception ex) {
            ex.printStackTrace();
         } 
